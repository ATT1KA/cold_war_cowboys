...
/// <summary>
/// Sprint 6 system: each cycle every rival faction takes one action, weighted
/// by their Agenda + relationship to the player + current standing. Mission
/// outcomes feed back into faction relationships through OnMissionResolved.
/// </summary>
public sealed class FactionSystem
{
	...
	public FactionSystem( Rng rng, EventBus bus, ConsequenceProcessor consequences )
	{
		_rng = rng;
		_bus = bus;
	...
	/// <summary>Runs one action per faction. Order is deterministic by faction id.</summary>
	...
	/// <summary>Reacts to a mission outcome by adjusting faction relationships.</summary>
	public void OnMissionResolved( CorporateState corp, MissionResolved result )
	{
		...
		foreach ( var fid in m.OpposesFactionIds )
		{
			...
			QueueRelationshipShift( f, -magnitude, $"Player op opposed our interests ({m.Name})" );
		}
		foreach ( var fid in m.AlliedFactionIds )
		{
			if ( !corp.Factions.TryGetValue( fid, out var f ) ) continue;
			QueueRelationshipShift( f, magnitude, $"Joint op succeeded ({m.Name})" );
		}
		if ( m.IssuingFactionId != null && corp.Factions.TryGetValue( m.IssuingFactionId, out var issuer ) )
		{
			QueueRelationshipShift( issuer, magnitude, $"Contract delivered ({m.Name})" );
		}
	}

	private FactionActionKind ChooseAction( Faction f, CorporateState corp )
	{
		// Each candidate gets a weight derived from agenda, relationship, standing.
		var candidates = new List<(FactionActionKind kind, double weight)>
		{
			(FactionActionKind.PoachOperative,    Weight( f, corp, FactionActionKind.PoachOperative )),
			(FactionActionKind.SabotageMission,   Weight( f, corp, FactionActionKind.SabotageMission )),
			(FactionActionKind.ProposeAlliance,   Weight( f, corp, FactionActionKind.ProposeAlliance )),
			(FactionActionKind.EscalateToBoard,   Weight( f, corp, FactionActionKind.EscalateToBoard )),
			(FactionActionKind.UndercutBudget,    Weight( f, corp, FactionActionKind.UndercutBudget )),
			(FactionActionKind.BankCash,          Weight( f, corp, FactionActionKind.BankCash )),
		};
		return _rng.PickWeighted( candidates );
	}

	private static double Weight( Faction f, CorporateState corp, FactionActionKind kind )
	{
		// Hostile actions scale with negative relationship; cooperative with positive.
		double rel = f.RelationshipToPlayer / 100.0;     // -1..+1
		double standing = f.Standing / 100.0;            // 0..1
		double cashFactor = Math.Min( 1.0, f.Cash / 200.0 );
		...
		// Hostile/expensive actions need cash + standing to even attempt.
		...
		switch ( kind )
		{
			case FactionActionKind.PoachOperative:
			{
				...
				if ( target != null && f.Cash >= cost )
				{
					...
					{
						Source = f.Name,
						Description = $"Poached operative {target.Name} from the player.",
						Apply = c =>
						{
			...
			case FactionActionKind.UndercutBudget:
			{
				_consequences.Enqueue( new Consequence
				{
					...
					Apply = c =>
					{
						c.Budget = Math.Max( 0, c.Budget - 50 );
						f.Cash += 25;
					},
				} );
				break;
	...
	private static Operative? PickPoachTarget( CorporateState corp )
	{
		...
		foreach ( var op in corp.Roster )
		{
			...
			if ( op.Loyalty < worstLoyalty )
			{
				worstLoyalty = op.Loyalty;
				candidate = op;
			}
		}
		return candidate;
