...
/// <summary>The kinds of one-off corporate events the generator can roll.</summary>
...
/// <summary>An event template loaded from JSON. Bindings are filled in at fire time.</summary>
public sealed class CorporateEventTemplate
{
	...
	/// <summary>Optional gating: "min_day:30" → won't roll before day 30.</summary>
	public List<string> Conditions { get; } = new();
}

/// <summary>Audit record published when an event fires.</summary>
public readonly record struct CorporateEventFired( CorporateEventKind Kind, string Narrative );

/// <summary>
/// Sprint 6 random-events layer. One event per cycle (or none) drawn from a
/// weighted pool. Conditions gate events; mechanical consequences are queued
/// through ConsequenceProcessor. Templates ship in JSON so designers can
...
public sealed class CorporateEventGenerator
{
	...
	/// <summary>Probability of rolling any event on a given cycle.</summary>
	public double FireProbability { get; set; } = 0.6;

	public CorporateEventGenerator( Rng rng, EventBus bus, ConsequenceProcessor consequences )
	{
		_rng = rng;
		_bus = bus;
	...
	/// <summary>Adds a template loaded from JSON.</summary>
	public void Register( CorporateEventTemplate t ) => _templates.Add( t );

	/// <summary>Replaces the entire template set (used by JSON loaders).</summary>
	...
	/// <summary>Rolls once. Most days nothing fires; sometimes a single event lands.</summary>
	public void Roll( CorporateState corp, WorldState world )
	{
		...
		var picked = _rng.PickWeighted( eligible );
		...
		switch ( t.Kind )
		{
			...
			case CorporateEventKind.BudgetCut:
				_consequences.Enqueue( new Consequence
				{
					Source = "Event",
					Description = narrative,
					Apply = c => c.Budget = Math.Max( 0, c.Budget - 200 ),
				} );
				break;
			...
			{
				int shift = _rng.Range( -150, 151 );
				_consequences.Enqueue( new Consequence
				{
					Source = "Event",
					Description = narrative + (shift >= 0 ? $" (+{shift})" : $" ({shift})"),
					Apply = c =>
					{
	...
	private (Faction a, Faction b)? PickTwoFactions( CorporateState corp )
	{
		...
		Faction b;
		do { b = _rng.Pick( list ); } while ( b.Id == a.Id );
		return (a, b);
	}

	private void SeedDefaultTemplates()
	{
		// JSON loader is the production path; these defaults exist so the
		// system runs without external data, useful for tests and bringup.
		Register( new CorporateEventTemplate
		{
			Id = "default_market_shift",
			Kind = CorporateEventKind.MarketShift,
			NarrativeTemplate = "Day {day}: market repositioning ripples through every division.",
			BaseWeight = 1.0,
		} );
	}
}
